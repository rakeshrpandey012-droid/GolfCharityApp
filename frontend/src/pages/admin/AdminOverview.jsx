...
export default function AdminOverview() {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      getLatestDraw().then(r => setDraw(r.data?.draw || r.draw || r.data || r)),
  ...
  return (
    <div style={{ paddingBottom: 60 }}>
      <Motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontFamily: "'Inter', sans-serif", fontSize: '2rem', fontWeight: 600, marginBottom: 8, color: 'var(--text-primary)' }}>
            System Analytics
          </h1>
      ...
      <Motion.div variants={stagger} initial="initial" animate="animate" className="bento-grid" style={{ marginBottom: 24 }}>
        {loading
          ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="bento-col-3" style={{ height: 160, background: 'var(--bg-subtle)', borderRadius: 24, animation: 'pulse 2s infinite' }} />)
          : stats.map((stat) => {
              ...
              return (
                <Motion.div key={label} variants={fadeUp} className="bento-card bento-col-3" style={{ padding: 24, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'relative', zIndex: 10 }}>
                    <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '1.8rem', fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.1, marginBottom: 6 }}>{value}</div>
      ...
      <div className="bento-grid">
        <Motion.div variants={fadeUp} initial="initial" animate="animate" className="bento-card bento-col-8" style={{ padding: 32, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, position: 'relative', zIndex: 10 }}>
            <div>
              <h3 style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: '1.25rem', color: 'var(--text-primary)', marginBottom: 4 }}>Live Engine Interface</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Tracking the current globally broadcasted draw sequence.</p>
            </div>
            {draw?.status === 'published' ? (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '4px 12px', background: 'var(--success-dim)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 99, color: 'var(--success)', fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase' }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--success)', boxShadow: '0 0 8px var(--success)' }} />
                Published
              </div>
            ) : (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '4px 12px', background: 'var(--warning-dim)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 99, color: 'var(--warning)', fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase' }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--warning)', boxShadow: '0 0 8px var(--warning)', animation: 'pulse 2s infinite' }} />
                Pending Action
              </div>
            )}
          </div>
          ...
          {loading ? (
            <div style={{ height: 100, background: 'var(--bg-subtle)', borderRadius: 16, animation: 'pulse 2s infinite' }} />
          ) : draw ? (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 40, alignItems: 'center', background: 'var(--bg-overlay)', padding: 32, borderRadius: 16, border: '1px solid var(--border)', position: 'relative', zIndex: 10 }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  {(draw.numbers || []).map((n, i) => (
                    <div key={i} style={{
                      ...
                      fontFamily: "'Inter', sans-serif", fontWeight: 700, color: 'var(--brand-vivid)', fontSize: '1.2rem',
              ...
              <div>
                ...
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '2rem', fontWeight: 600, color: 'var(--brand-vivid)' }}>£{(draw.totalPool || 0).toFixed(2)}</div>
              </div>
            </div>
          ) : (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, background: 'var(--bg-overlay)', borderRadius: 16, border: '1px dashed var(--border)' }}>
        ...
        <Motion.div variants={fadeUp} initial="initial" animate="animate" className="bento-card bento-col-4" style={{ padding: 32, display: 'flex', flexDirection: 'column' }}>
          <h3 style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: '1.25rem', color: 'var(--text-primary)', marginBottom: 24 }}>Control Protocols</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 12, background: item.bg }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-subtle)' }}>
                ...
                <button onClick={() => navigate(item.path)} className="btn btn-ghost" style={{ padding: '8px 12px', minHeight: 'unset', fontSize: '0.82rem' }}>Open</button>
              </div>
            ))}
          </div>
        </Motion.div>
